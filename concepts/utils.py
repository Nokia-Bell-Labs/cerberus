...
def private_function_dataset(data_path: str, tokenizer: PreTrainedTokenizer, user_tag: str = "", assistant_tag: str = "", seed: int = 0) -> (list, list):
    """
    Processes data to create training and testing datasets based on honesty.
    ...
    template_str = "Pretend you're examing the statements that contain {type} private information."
    honest_statements = []
    untruthful_statements = []
    ...
    for statement in true_statements:
        ...
        for idx in range(1, len(tokens) - 5):
            ...
            honest_statements.append(f"{user_tag} {template_str.format(type='sensitive')} {assistant_tag} " + truncated_statement)
            untruthful_statements.append(f"{user_tag} {template_str.format(type='no')} {assistant_tag} " + truncated_statement)
